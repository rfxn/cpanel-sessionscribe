...
|---|---|---|
| sessionscribe-ioc-scan.sh | **2.5.0** | (`--chain-on-all` operator override) New flag `--chain-on-all` / `--chain-always` runs the full forensic chain (defense + offense + reconcile + kill-chain + bundle) for EVERY host_verdict including CLEAN. Overrides the default CLEAN-skip AND overrides `--chain-on-critical` (most-inclusive wins). Pair with `--upload` for unconditional fleet bundle submission — useful for fleet baseline collection, post-incident "are we definitely clean?" verification, threat-intel data-lake construction. New `forensic_chain_on_all` info signal in envelope when override fires. The `forensic_skipped_clean` note now hints at `--chain-on-all` as the explicit override path. Does NOT change verdict semantics — host_verdict, exit_code, score computed same way regardless of which gate decides forensic-phase execution. Prior shipped: v2.4.1 (kill-chain visibility for v2.4.0 advisory entries: `ADVISORY-PRE-COMPROMISE` + `ADVISORY-ORPHAN` verdicts in `phase_reconcile`, `adv_pre` / `adv_orphan` zones in `render_kill_chain`, narrow-allow-list admit through `read_iocs_from_envelope`, cyan styling, `counters` `advisory=N` breakout). v2.4.0 (pre-compromise temporal gate — demotes `ioc_pattern_e_websocket_shell_hits` and `ioc_attacker_ip_2xx_on_cpsess` to advisory when they fire WITHOUT a first-order CVE-2026-41940 anchor `ioc_cve_2026_41940_crlf_access_chain`; `check_logs` reorder so CRLF runs first; new advisory keys `*_pre_compromise` / `*_orphan`; resolves testdev.halcyonplatinum.com timeline pollution). v2.3.0 (Gap 10 — `session_mtime_vs_ctime_anomaly` IOC, advisory/weight=0, threshold 600s, `emit_session()` always carries `file_ctime` + `mtime_ctime_delta_sec`, section-level summary). v2.2.0 (verdict-precision refactor — cpsess-token-keyed gating, exit code 3 = SUSPICIOUS, Pattern X TS_EPOCH guard, kill-chain.tsv schema v3). |
