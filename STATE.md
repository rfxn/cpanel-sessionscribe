...
|---|---|---|
| sessionscribe-ioc-scan.sh | **2.3.0** | (Gap 10: mtime/ctime forensic flag) Adds `session_mtime_vs_ctime_anomaly` IOC (advisory/weight=0): flags sessions where `|mtime - ctime| >= 600s`. cpsrvd writes both timestamps atomically so legitimate sessions have `mtime == ctime` to the second; divergence is `touch -d` backdating or `cp -p` / `tar xp` / `rsync -t` restore artifact. Resolves `testdev.halcyonplatinum.com` pre-disclosure outlier — the 2025-11-25 first-X timestamp that anchored cluster-onset 4.5 months before campaign start was a session whose mtime was 5+ months earlier than ctime; downstream `ss-aggregate.py q5/q8` can now discount mtime when this IOC fires. `emit_session()` always carries `file_ctime` + `mtime_ctime_delta_sec` so aggregators can compute the delta per-emit without joining rows. Section-level summary `session_mtime_anomaly_summary` (advisory) emits per-host count for fleet-wide restore-artifact detection. `no_session_iocs` all-clear is now gated on `mtime_anomalies == 0`. Severity is `advisory` (not `warning`) so `cp -p` / `tar xp` restore artifacts on a clean host do NOT escalate `host_verdict`. Single `stat -c '%Y %Z'` call replaces prior `stat -c %Y` (no new subprocesses). Prior shipped: v2.2.0 (verdict-precision refactor — cpsess-token-keyed gating, exit code 3 = SUSPICIOUS, Pattern X TS_EPOCH guard, kill-chain.tsv schema v3). |
