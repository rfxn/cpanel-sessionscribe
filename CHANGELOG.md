...
severity only. 300-envelope projection: 0 hosts moved COMPROMISED →
CLEAN; 204 recon-only hosts moved SUSPICIOUS → CLEAN per
`feedback_noise_does_not_gate` (failed-exploit / recon / quarantined-
only floods stay CLEAN).

### Maintenance (commit ba98e12)

Comment compaction + dead-code trim per CLAUDE.md commenting discipline.
12 multi-line v2.7.32 P-tag comment blocks compacted to one-line
CHANGELOG anchors. 7 long-prose rationale blocks collapsed or
removed. Three dead-code residues dropped (unused `_letter`
assignment in P3 pre-pass, duplicated `weight="${weight:-0}"` reset,
dead `|| ""` half of P3 weight check). Net −67 lines, no behavior
change.
